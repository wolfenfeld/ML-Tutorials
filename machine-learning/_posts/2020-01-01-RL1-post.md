...
In this framework, I have implemented several _agents_ and _worlds_ where the agent will play, to be used as reference or as a benchmark.
...
The agent job is very simple, give the state and what we have already learned - what is the right action to take.
...
The _Decision Model_ holds the current policy which is implements as a _F_ by _A_ weight matrix. 
Where _F_ is the number of _features_ (4 in our case) and _A_ is the number of available actions.

Whenever the agent needs to take an action, it is given a state comprised of the _F_ features.
The best action according to the current policy is chosen with the following function:
...
The decision model maintains the the _best_reward_, 
which is the best cumulative reward that the agent has achieved in a an episode. 
And the _best_weights_ which is the weights matrix that achieved the _best_reward_.

At the end of each episode, once the decision model is updated. 
...
In this post we have started with what is Reinforcement Learning, how it differs form other machine learning paradigms.
We have introduced the _Carrot and Stick_ framework and showed an example algorithm - _Hill CLimb_.
